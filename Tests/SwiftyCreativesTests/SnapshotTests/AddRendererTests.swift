...
#if os(macOS)
final class AddRendererTests: XCTestCase {
    ...
    func testAddRendering() async throws {
        ...
        class TestSketch: Sketch {
            ...
            override func afterCommit(texture: MTLTexture?) {
                ...
                assertSnapshot(matching: finalimage, as: .image, record: SnapshotTestUtil.isRecording, testName: "testAddRendering")
            }
        }
        
        let expectation = XCTestExpectation()
        let sketch = TestSketch(expectation)
        let swiftuiView = SketchView(sketch, blendMode: .add)

...
final class ImgTests: XCTestCase {
    ...
    @MainActor
    func testImgIsDrawed() async throws {
        try SnapshotTestUtil.testGPU()
        class TestSketch: SketchForTest {
            let testImage = Img().load(name: "sampleImage", bundle: .module).adjustScale(with: .basedOnWidth).multiplyScale(6)
        ...
        let sketch = TestSketch(expectation, testName: "testImgIsDrawed")

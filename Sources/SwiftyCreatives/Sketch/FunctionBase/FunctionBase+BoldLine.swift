...
public extension FunctionBase {
    ...
    func boldline(_ x1: Float, _ y1: Float, _ z1: Float, _ x2: Float, _ y2: Float, _ z2: Float, width: Float, color1: f4, color2: f4) {
        let diffY = abs(y2 - y1)
        let diffX = abs(x2 - x1)
        if diffX <= 0.00001 {
            let corner1 = f3(x1 - width, y1, z1)
        ...
        setVertexColors([color1, color1, color2, color2])
        privateEncoder?.drawPrimitives(type: .triangleStrip, vertexStart: 0, vertexCount: 4)
    }
    
    func boldline(_ pos1: f3, _ pos2: f3, width: Float, color1: f4, color2: f4) {
        boldline(pos1.x, pos1.y, pos1.z, pos2.x, pos2.y, pos2.z, width: width, color1: color1, color2: color2)
    }
}
